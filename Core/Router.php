...
class Router
{
    ...
    private function ValidateRoute($RouteUrl): bool
    {
        ...
        // $Uri = array_values(array_filter(explode('/', "longgoal/{$RouteUrl}")));
        $Uri = array_values(array_filter(explode('/', $RouteUrl))); //-- For live server
        // $Uri = array_values(array_filter(explode('/', "folder/{$RouteUrl}"))); //-- For local folder
